...
This interface connects to our high-performance FastAPI backend,
running a physics-grounded Graph Neural Network.
The model utilizes a Three-Body Message Passing architecture with
Bessel Radial Basis and Chebyshev Angle Basis functions to predict
Bandgap and Formation Energy directly from raw `.cif` files.
""")
...
with col1:
    ...
    if uploaded_file is not None and st.button("Run Prediction", type="primary"):
            with st.spinner("Processing through ONNX Runtime..."):
                try:
                    # Send to FastAPI
                    files = {"file": (
                        uploaded_file.name, uploaded_file.getvalue(), "chemical/x-cif"
                    )}
                    ...
                    if response.status_code == 200:
                        res = response.json()
                        st.success(
                            f"Success! Inference took {res['inference_time_ms']} ms "
                            f"using {res['backend']}."
                        )

                        mcol1, mcol2 = st.columns(2)
                        mcol1.metric("Bandgap (eV)", f"{res['bandgap_eV']:.4f}")
                        e_val = res['formation_energy_eV_per_atom']
                        mcol2.metric("Energy (eV/atom)", f"{e_val:.4f}")
                    else:
